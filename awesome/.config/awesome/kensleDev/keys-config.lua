...
local rofi = {}
function rofi.run() awful.util.spawn("rofi -show run -theme purple") end
function rofi.drun() awful.util.spawn("rofi -show drun -theme purple") end
function rofi.show() awful.util.spawn("rofi -show -theme purple") end
...
function hotkeys:init(args)
  ...
  -----------------------------------------------------------
	...
	local apprunner_keys_move = {
		...
		{
			{ env.mod }, "l", function() apprunner:up() end,
	...
	local menu_keys_move = {
		...
		{
			{ env.mod }, "l", redflat.menu.action.up,
			{ description = "Select previous item", group = "Navigation" }
		},
		{
		...
		{
			{ env.mod }, ";", redflat.menu.action.enter,
			{ description = "Open submenu", group = "Navigation" }
		},
	  {
			{ env.mod }, "u", function () awful.tag.incmwfact(0.05) end,
			{ description = "Increase master width factor", group = "Layout" }
		},
		{
			{ env.mod }, "p", function () awful.tag.incmwfact(-0.05) end,
			{ description = "Decrease master width factor", group = "Layout" }
		},
  }
	...
	self.raw.root = {
		{
			{ env.mod, "Shift" }, "s", function() redtip:show() end,
	  ...
	  {
			{ env.mod, "Shift" }, ";", function () awful.tag.incmwfact(0.05) end,
			{ description = "Increase master width factor", group = "Layout" }
		},
		{
			{ env.mod, "Shift" }, "j", function () awful.tag.incmwfact(-0.05) end,
			{ description = "Decrease master width factor", group = "Layout" }
		},
		{
			{ env.mod }, ";", focus_switch_byd("right"),
		...
		{
			{ env.mod }, "space", function() rofi.run() end,
		...
		-- {
		-- 	{ env.mod, "Shift" }, "a", nil, function() appswitcher:show({ filter = current, reverse = true }) end,
		...
		{
			{ env.mod }, "d", awful.tag.viewnext,
			{ description = "View next tag", group = "Tag navigation" }
		},
		{
			{ env.mod }, "s", awful.tag.viewprev,
		...
		-- },
		{
			{ env.mod }, "y", function() awful.layout.inc(1) end,
			{ description = "Select next layout", group = "Layouts" }
		},
		{
			{ env.mod, "Shift" }, "y", function() awful.layout.inc(-1) end,
	...
	self.raw.client = {
		...
		{
			{ env.mod }, "v", function(c) c:kill() end,
